...
class BaseLearner(object):
    ...
    def save_checkpoint(self, filename):
        self._network.cpu()
        torch.save(self._network.state_dict(), '{}/model_step{}.ckpt'.format(filename, self._cur_task))
        # save exemplar
        memory = {
            'x': self._data_memory,
            'y': self._targets_memory,
            'm': self._class_means
        }
        torch.save(memory, "{}/mem_step{}.ckpt".format(filename, self._cur_task))
        logging.info(f"Save step{self._cur_task} model and memory!")
    ...
    def _evaluate(self, y_pred, y_true):
        ...
        topk = self.topk if self._total_classes >= self.topk else self._total_classes
        ret['top{}'.format(self.topk)] = np.around((y_pred.T == np.tile(y_true, (topk, 1))).sum()*100/len(y_true),
                                                   decimals=2)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            topk = self.topk if self._total_classes >= self.topk else self._total_classes
            predicts = torch.topk(outputs, k=topk, dim=1, largest=True, sorted=True)[1]  # [bs, topk]
    ...
    def _eval_nme(self, loader, class_means):
        ...
        return np.argsort(scores, axis=1)[:, :topk], y_true  # [N, topk]
    ...
    def _construct_exemplar(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            exemplar_targets = np.full(m_, class_idx)
